...
for fold in folds_list:
    ...
    if train_or_test == 'train':
        for C in C_cands:
            clf = LinearSVC(random_state=args.randSeed, C=C, dual=False, max_iter=5000).fit(data_train, label_train)
            ...
            if val_acc > val_acc_best:
                ...
                best_C = C
                model_save_path = os.path.join(model_dir, 'subject_%s_vids_%s_fold_%s_valid_%s.joblib' %(subjects_type,str(n_vids),str(fold),valid_method))
    ...
    if train_or_test == 'test':
        model_path = os.path.join(model_dir,'subject_%s_vids_%s_fold_%s_valid_%s.joblib' %(subjects_type,str(n_vids),str(fold),valid_method))
...
if train_or_test == 'train':
    ...
    pd.DataFrame(subjects_score).to_csv(os.path.join(result_dir, 'subject_acc.csv'))
