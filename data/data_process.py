...
def LDS(sequence, V0=0.01, T=0.0001, sigma=1.0, given_all=1):
    # shape: (timeSample, n_dims). The original implementation filtered each
    # feature independently; this keeps the same diagonal Kalman equations while
    # vectorizing across feature dimensions.
    sequence = np.asarray(sequence)
    if sequence.ndim != 2:
        raise ValueError(f"LDS expects a 2D array, got shape {sequence.shape}")
    ...
    A = 1.0
    C = 1.0
    givenAll = int(given_all)

    u0 = np.mean(sequence, axis=0)
    P = np.zeros((n_dims, n), dtype=sequence.dtype)
    u = np.zeros((n_dims, n), dtype=sequence.dtype)
    V = np.zeros((n_dims, n), dtype=sequence.dtype)
    K = np.zeros((n_dims, n), dtype=sequence.dtype)
    ...
    if givenAll == 1:
        uAll = np.zeros((n_dims, n), dtype=sequence.dtype)
        VAll = np.zeros((n_dims, n), dtype=sequence.dtype)
        uAll[:, n - 1] = u[:, n - 1]
        VAll[:, n - 1] = V[:, n - 1]
        ...
        for ir in range(n - 1):
            i = n - 2 - ir
            J = V[:, i] * A / P[:, i]
            uAll[:, i] = u[:, i] + J * (uAll[:, i + 1] - A * u[:, i])
            VAll[:, i] = V[:, i] + J * (VAll[:, i + 1] - P[:, i]) * J
