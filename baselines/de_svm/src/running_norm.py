...
for decay_rate in [0.990]:
    ...
    # for fold in range(n_folds-1, n_folds):
        ...
        if use_features == 'de':
            ...
            # 原代码写死 30*4=120 维,而 5 频带数据为 30*5=150 维,reshape 会因元素数不匹配报错。
            # 保持原 transpose 维度顺序不变,仅按实际通道数与频带数动态计算展平维度。
            n_chans = data.shape[1]            # de_features.mat: (n_subs, 30ch, 840, n_bands)
            n_bands = data.shape[3]            # = 5
            data = data.transpose([0, 2, 3, 1]).reshape(n_subs, 840, n_chans * n_bands)
            if n_vids == 24:
