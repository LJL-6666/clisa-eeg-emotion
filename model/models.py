...
class ConvNet_baseNonlinearHead(nn.Module):
    """
    CLISA-style encoder adapted to the DAEST/Lightning interface.

    `saveFea=True` returns the temporal feature map used by `extract_fea.py`.
    `saveFea=False` returns the flattened projector output for contrastive pretraining.
    """
    ...
    def __init__(
        ...
        self.spatialConv2 = nn.Conv2d(
            n_timeFilters,
            n_timeFilters * multiFact,
            (n_spatialFilters, 1),
            groups=n_timeFilters,
        )
        self.timeConv2 = nn.Conv2d(
            n_timeFilters * multiFact,
            n_timeFilters * multiFact * multiFact,
            (1, timeSmootherLen),
    ...
    def forward(self, input):
        ...
        if self.isMaxPool:
            ...
            pooled = out.new_zeros((out.shape[0], out.shape[1] // 2, out.shape[2], out.shape[3]))
            for i in range(out.shape[0]):
                pooled[i, :, :, :] = out[i, indices[i, :, 0]]
            proj_out = pooled.reshape(pooled.shape[0], -1)
        else:
...
class Conv_att_transformer(nn.Module):
    ...
    def __init__(self, n_timeFilters, timeFilterLen, n_msFilters, msFilter_timeLen, n_channs=64, dilation_array=np.array([1,6,12,24]), avgPoolLen = 30,
        ...
        super().__init__()
        if BertModel is None or BertConfig is None:
            raise ImportError("transformers is required to use Conv_att_transformer")
        self.stratified = stratified
        self.msFilter_timeLen = msFilter_timeLen
        self.activ = activ
        self.temp = temp
