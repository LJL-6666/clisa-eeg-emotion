...
This file keeps the original repository result and the new local full runs side by side.
The original code/results are preserved. New launchers and run directories are kept separately.
...
### 0.05-47 Hz Processed_data Branch
...
The repository reference result and the new 6-GPU runs should not be treated as identical execution protocols:

- The preserved repository result represents the reference single-run result stored under `results/`.
