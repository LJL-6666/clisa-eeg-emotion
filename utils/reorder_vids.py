...
def _normalize_after_remarks_dir(path_like):
    ...
    if path.is_file():
        if path.name == "After_remarks.mat":
    ...
    candidates = (
        path,
        path / "After_remarks",
        path / "Clisa_analysis" / "After_remarks",
        path / "FACED_Data" / "Clisa_analysis" / "After_remarks",
        path.parent / "After_remarks",
        path.parent / "Clisa_analysis" / "After_remarks",
    )
    for cand in candidates:
        if cand.exists() and cand.is_dir() and any(cand.glob("sub*/After_remarks.mat")):
            return cand
    return None


def resolve_after_remarks_dir(data_root=None, after_remarks_dir=None):
...
def video_order_load(n_vids=28, data_root=None, after_remarks_dir=None):
    datapath = resolve_after_remarks_dir(data_root=data_root, after_remarks_dir=after_remarks_dir)
