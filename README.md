...
| 环境 | 覆盖 | 配方文件 | 备注 |
|---|---|---|---|
| **CPU 环境**（Python 3.10） | 预处理 + DE+SVM | [`Svm_analysis/requirements.txt`](Svm_analysis/requirements.txt) | 预处理依赖是其严格子集（版本全同），复用同一环境即可 |
| **GPU 环境**（Python 3.11 + CUDA） | CLISA | [`Clisa_analysis/environment.yml`](Clisa_analysis/environment.yml) | 需 NVIDIA GPU；版本与 CPU 栈不同，独立 |
