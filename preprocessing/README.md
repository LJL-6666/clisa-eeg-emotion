...
与 [clisa-eeg-emotion](https://github.com/LJL-6666/clisa-eeg-emotion) 并存：训练与评测分别在 [`Clisa_analysis/`](../Clisa_analysis/)（CLISA）与 [`Svm_analysis/`](../Svm_analysis/)（DE+SVM）；此处负责预处理 **原始 FACED EEG → 逐被试 pkl**。
...
3. **勿混淆**：仓库根目录**没有**统一的 `main.py`。CLISA 训练入口在 [`Clisa_analysis/main.py`](../Clisa_analysis/main.py)，DE+SVM 在 [`Svm_analysis/src/`](../Svm_analysis/src/)；本目录的 `main.py` 仅用于 EEG 预处理。
